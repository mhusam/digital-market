    ...
    <div>
      ...
      {items === null ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {items.map((c, i) => (
            <div key={c.id} className="bg-white border border-[#e8e5df] rounded-xl p-5 hover:border-[#d8d4cc] transition-colors">
              <div className="flex items-start justify-between mb-3">
                <span
                  className="size-10 rounded-lg flex items-center justify-center text-[#1B1B1B] font-bold text-[15px]"
                  style={{ background: accents[i % accents.length] + "33", color: "#1B1B1B" }}
                >
                  {c.name.charAt(0)}
                </span>
              ...
              <p className="text-[12.5px] text-[#6b6760] mt-0.5 font-mono">/{c.slug}</p>
              {c.description && <p className="text-[13px] text-[#3a3833] mt-2 line-clamp-2">{c.description}</p>}
