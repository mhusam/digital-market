...
import { RouteBreadcrumb } from "@/components/ui/route-breadcrumb";
  ...
  return (
    <div className={styles.postTags} aria-label="Post tags">
      {post.tags.map((tagSlug) => {
        ...
        if (!tag) return null;
        return (
          <Link key={tag.slug} href={`/blog?tag=${tag.slug}`} data-tone={tag.tone}>
            {tag.label}
          </Link>
        );
      })}
...
export default async function BlogPostPage({ params }: BlogPostPageProps) {
  ...
  return (
    <article className={`page-container ${styles.articleShell}`} aria-labelledby="article-title">
      <RouteBreadcrumb
        items={[
          { href: "/", label: "Home" },
      ...
      <div className={styles.articleLayout}>
        <aside className={styles.articleAside} aria-label="Article table of contents">
          <div className={styles.articleAsideInner}>
            <section className={styles.articleAsidePanel}>
              ...
              {toc.length > 0 ? (
                ...
                <p className={styles.tocEmpty}>This note has no section headings.</p>
              )}
            </section>

            <section className={styles.articleAsidePanel}>
              <p className={styles.railTitle}>Article</p>
              <dl className={styles.articleStats}>
                <div>
                  <dt>Author</dt>
                  <dd>{author.name}</dd>
                </div>
