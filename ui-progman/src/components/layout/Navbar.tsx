...
function Logo() {
  return (
    <span className="group inline-flex items-center gap-2.5 font-extrabold text-foreground">
      ...
      />
      <span className="font-brand text-[1.08rem] tracking-[0.03em] brand-shimmer">
        PROGMAN
      </span>
    </span>
  );
...
export function Navbar() {
  ...
  useEffect(() => {
    const handleGlobalKeyDown = (e: KeyboardEvent) => {
      // Toggle launcher on Cmd+K or Ctrl+K
  ...
  useEffect(() => {
    if (launcherOpen) {
      ...
      // Auto-focus search input after transition completes
  ...
  // Filter items in the App Launcher
  const filteredLauncherItems = LAUNCHER_ITEMS.filter((item) => {
    const matchesSearch =
  ...
  return (
    <>
          ...
          <div className="flex h-16 items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-1.5">
              ...
              <button
                ...
                className={`relative z-50 ml-1 inline-flex h-10 items-center gap-2 rounded-full border px-3.5 text-[13px] font-bold cursor-pointer transition-all ${
                  launcherOpen
              ...
              {!launcherOpen && (
                <button
                  type="button"
                  className="inline-flex size-10 items-center justify-center rounded-full border border-border bg-card/60 text-foreground md:hidden cursor-pointer"
      ...
      >
        ...
        <div className="page-container relative flex min-h-screen flex-col pt-16 pb-12">
          <header className="mb-8 flex items-end justify-between gap-6 border-b border-border pb-6">
            ...
            <div className="hidden text-right md:block">
              <p className="text-[10px] font-bold uppercase tracking-[0.22em] text-muted-foreground">
                {LAUNCHER_ITEMS.length} shortcuts · {LAUNCHER_SECTIONS.length} sections
              </p>
              <p className="mt-2 font-mono text-xs text-muted-foreground">
                Press <kbd className="rounded-md border border-border bg-card px-1.5 py-0.5 text-[10px] font-bold">ESC</kbd> to close
              </p>
            </div>
          </header>
          ...
          <div className="mb-8 flex items-center gap-3 rounded-2xl border border-border bg-card/70 px-4 py-3.5 backdrop-blur">
            ...
            <span className="hidden items-center gap-1 font-mono text-[10px] font-bold uppercase tracking-[0.18em] text-muted-foreground sm:inline-flex">
              <kbd className="rounded-md border border-border bg-background px-1.5 py-0.5">⌘</kbd>
              <kbd className="rounded-md border border-border bg-background px-1.5 py-0.5">K</kbd>
            </span>
          </div>
          ...
          <div className="mb-6 flex flex-wrap items-center gap-2 text-[11px] font-bold uppercase tracking-[0.18em]">
            ...
            <button
              ...
              className={`rounded-full border px-3 py-1 cursor-pointer transition-colors ${
                activeFilter === "All"
                  ? "border-foreground bg-foreground text-background"
          ...
          <div className="grid flex-grow grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4">
            {filteredLauncherItems.map((item, idx) => {
          ...
          <footer className="mt-10 flex flex-col gap-3 border-t border-border pt-6 text-[11px] font-bold uppercase tracking-[0.18em] text-muted-foreground md:flex-row md:items-center md:justify-between">
            ...
            <span>
              PROGMAN Universe · v{new Date().getFullYear()}.{new Date().getMonth() + 1}
            </span>
          </footer>
        </div>
      </div>
